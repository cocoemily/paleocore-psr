...
def parse_access(file_path, site_name, locality_names=locality_names):
    ...
    if gcmatch is None:
        ...
        try:
            locality = GeologicalContext.objects.get(name=gcmatch)
        except:
            if gcmatch in locality_names:
                lname2 = locality_names[gcmatch]
                locality = GeologicalContext.objects.get(name=lname2)
            elif not any(char.isdigit() for char in lname):
                lname2 = gcmatch + " 1"
                locality = GeologicalContext.objects.get(name=lname2)
    ...
    for u in set(context["Unit"]):
        if u is not " ":
            psr_eu = ExcavationUnit.objects.get_or_create(unit=u, geological_context=locality)[0]
            # psr_eu.extent = MultiPoint(Point(int(u[2]), int(u[4]), srid=-1), Point(int(u[3]), int(u[5]), srid=-1), srid=-1)
            psr_eu.save()
    ...
    for i in range(0, len(context["ID"])):
        ...
        psr_eo.type = context["code"][i]
        psr_eo.excavator = context["excavator"][i]
        psr_eo.cat_number = context["Unit"][i] + " " + context["ID"][i]

        mdb_name = context["excavator"][i]
        full_name = [val for key, val in PERSON_DICTIONARY.items() if mdb_name.lower().capitalize() in key]
        if full_name.__len__() != 0:
            name = full_name[0].split(" ")
            psr_eo.found_by = Person.objects.get_or_create(last_name=name[1], first_name=name[0])[0]
    ...
    for i in range(0, len(xyz["ID"])):
        ...
        if obj.prism is None:
            ...
            prism_list = obj.prism
        prism_list.append(xyz["Prism"][i])
        obj.prism = prism_list

        if (xyz["Date"][i] + " " + xyz["Time"][i]) is not " ":
            obj.date_collected = datetime.strptime(xyz["Date"][i] + " " + xyz["Time"][i], '%m/%d/%Y %I:%M:%S %p')

        if not is_new:
            if obj.point is not None:
                new_point = Point(float(xyz["X"][i]), float(xyz["Y"][i]), float(xyz["Z"][i]), srid=-1)
                points = [pt for pt in obj.point]
                if new_point not in points:
                    points.append(new_point)
                    obj.point = MultiPoint(points, srid=-1)
            else:
                obj.point = MultiPoint(Point(float(xyz["X"][i]), float(xyz["Y"][i]), float(xyz["Z"][i]), srid=-1))
        else:
            obj.point = MultiPoint(Point(float(xyz["X"][i]), float(xyz["Y"][i]), float(xyz["Z"][i]), srid=-1))
